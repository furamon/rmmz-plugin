 ...
 *  - Effect type -1 does not allow the map screen to be used as
 *    the background.
 *
 * ================================
 ...
 * @author 木星ペンギン(改変:Furamon)
 * @url
 *
 * @help [version 4.2.2-p7]
 ...
 *
 * ▼ その他
 *  - エフェクトタイプ 1 にて、中心となるキャラクターを指定していない場合、
 *    ランダムエンカウントでは[プレイヤー]、
 *    イベントコマンドの【戦闘の処理】では[このイベント]が中心となります。
 *  - エフェクトタイプ -1 ではマップ画面を背景にはできません。
 *
 * ================================
...
(() => {
    ...
    const pluginName = 'Furamon_M_EncounterEffect_pixiv7';
    ...
    EncounterEffect.isRunning = function() {
        return this._fadeSign !== 0;
    };

    EncounterEffect.isFadeOut = function() {
        return this._fadeSign < 0;
    };

    EncounterEffect.isFadeIn = function() {
        return this._fadeSign > 0;
    };

    EncounterEffect.isReady = function() {
        return !this.isRunning() || this._duration < 45;
    };

    EncounterEffect.isSpritesetVisible = function() {
        return this._opacity < 255 || this._backOpacity < 255;
    };
    ...
    EncounterEffect.backOpacity = function() {
    ...
    EncounterEffect.startFadeOut = function() {
        this._fadeSign = -1;
        this._duration = this.effectSpeed();
        this._opacity = 255;
        this._backOpacity = 255;
        this.createFragments();
        this.sortFragment(this.currentParam('Break Direction'));
        this.startFadeOutAngle();
    };

    EncounterEffect.startFadeOutAngle = function() {
        this._angle = 0;
        this._targetAngle = 0;
        ...
        if (this._type === 5) {
            ...
            this._targetAngle = -3;
            this._angleEasing.start('Slow end', speed);
        }
    };
    ...
    EncounterEffect.generatorPointsT2 = function*(allPoints) {
        ...
        for (let i = 0; i < splitX; i++) {
            ...
            for (let j = 0; j < splitY; j++) {
                ...
                if (sType === 'triangle') {
                    if (i % 2 === 0) {
                        yield [point1, point2, point3];
                        yield [point1, point3, point4];
                    } else {
                        yield [point1, point2, point4];
                        yield [point2, point3, point4];
                    }
    ...
    EncounterEffect.startFadeIn = function() {
        if (!this.isFadeOut()) {
            return;
        }
        this._fadeSign = 1;
        ...
        this.startFadeInFragments(duration, delay);
        this.startFadeInAngle(duration);
    };

    EncounterEffect.startFadeInFragments = function(duration, delay) {
        const maxItems = this.maxItems();
        for (const [i, fragment] of this._fragments.entries()) {
            fragment.startFadeIn(duration, delay);
            fragment.setDelay(Math.floor(delay * i / maxItems));
        }
    };

    EncounterEffect.startFadeInAngle = function(duration) {
        if (this._type === 5) {
            this._targetAngle = -4;
    ...
    EncounterEffect.update = function() {
        if (this.isRunning()) {
            ...
            this.updateAngle();
            if (this.isFadeIn() && this._duration === 0) {
                this.clear();
            }
        }
    };

    EncounterEffect.updateMain = function() {
        if (this._duration > 0) {
            if (this.isFadeOut()) {
                const speed = this.effectSpeed();
                const n = speed - this._duration;
                const breakEnd = this.breakDuration();
                if (n > 2 && n <= breakEnd + 2) {
                    const maxItems = this.maxItems();
                    const start = Math.floor(maxItems * (n - 3) / breakEnd);
                    const end = Math.floor(maxItems * (n - 2) / breakEnd);
                    this.breakFragments(start, end, speed);
                }
            }
            this._duration--;
        }
    ...
    EncounterEffect.updateOpacity = function() {
        if (this.isFadeIn()) {
            ...
            const speed = this.effectSpeed();
            const delay = this._type === 5 ? speed - 1 : Math.floor(speed / 2);
            if (d < speed - delay) {
                this._opacity -= this._opacity / d;
            }
        }
    };

    EncounterEffect.updateBackOpacity = function() {
        if (this.isFadeIn()) {
            const d = this._duration;
            const speed = this.effectSpeed();
            const delay = Math.floor(speed / 3);
            if (d < speed - delay) {
                this._backOpacity -= this._backOpacity / d;
            }
        }
    };
    ...
    class Encounter_Fragment {
        ...
        flashColor() {
            return [...EncounterEffect.color(), this.flashOpacity()];
        }
        ...
        startFadeIn(duration, delay) {
            switch (EncounterEffect.type()) {
                case 1:
                    this.startFadeInT1(duration);
                    break;
                case 2:
                    this.startFadeInT2(Math.max(duration - delay, 1));
                    break;
                case 3:
                    this.startFadeInT3(duration);
                    break;
                case 4:
                    this.startFadeInT4(duration);
                    break;
                case 5:
                    this.startFadeInT5(Math.max(duration - delay, 1));
                    break;
            }
        }

        startFadeInT1(duration) {
            this._moveEasing.start('Slow end', duration);
    ...
    class Graphics_Fragment extends PIXI.Graphics {
        ...
        refresh() {
            ...
            if (this._break && lw > 0) {
                ...
                this.lineStyle({ width: lw + 0.5, color, alpha: 0.25, alignment: 0.3 });
                this.drawPolygon();
                this.lineStyle({ width: lw, color, alpha: 0.5, alignment: 0.3 });
                this.drawPolygon();
            }
        }
    ...
    class Sprite_EncounterEffect extends PIXI.Container {
        ...
        updateOpacity() {
            ...
            this._backGraphics.alpha = EncounterEffect.backOpacity() / 255;
        }
    ...
    Spriteset_Map.prototype.showDestination = function() {
        this._destinationSprite.visible = true;
    };

    Spriteset_Map.prototype.hideDestination = function() {
    ...
    Scene_Base.prototype.createEncEffSprite = function() {
        if (EncounterEffect.isRunning() && !this._encEffSprite) {
    ...
    Scene_Base.prototype.startEncEffFadeOut = function() {
        if (!EncounterEffect.isRunning()) {
            this.snapEncEffTexture();
            EncounterEffect.startFadeOut();
            this.createEncEffSprite();
        }
    };

    Scene_Base.prototype.snapEncEffTexture = function() {
        this._windowLayer.visible = false;
        EncounterEffect.setup(SceneManager.snapTexture());
        this._windowLayer.visible = true;
    };

    Scene_Base.prototype.startEncEffFadeIn = function() {
        if (EncounterEffect.isFadeOut()) {
            EncounterEffect.startFadeIn();
            this.createEncEffSprite();
        }
    };

    Scene_Base.prototype.updateMppEncEffManager = function() {
        if (EncounterEffect.isRunning()) {
            EncounterEffect.update();
            this._spriteset.visible = EncounterEffect.isSpritesetVisible();
            if (!EncounterEffect.isRunning()) {
    ...
    const _Scene_Map_startFadeIn = __base(Scene_Map.prototype, 'startFadeIn');
    Scene_Map.prototype.startFadeIn = function(duration, white) {
        if (!EncounterEffect.isRunning()) {
            _Scene_Map_startFadeIn.apply(this, arguments);
        }
    };
    ...
    Scene_Map.prototype.snapEncEffTexture = function() {
        this._spriteset.hideDestination();
        Scene_Message.prototype.snapEncEffTexture.call(this);
        this._spriteset.showDestination();
    };
    ...
    Scene_Map.prototype.updateMppEncounterEffect = function() {
        if (this._encounterEffectDuration > 0) {
            ...
            if (n === 2) {
                this._spriteset.visible = true;
                this._encEffSprite.visible = false;
                this.snapForBattleBackground();
                this._encEffSprite.visible = true;
            }
    ...
    Scene_Battle.prototype.create = function() {
        ...
        this.startEncEffFadeIn();
    };

    const _Scene_Battle_startFadeIn = __base(Scene_Battle.prototype, 'startFadeIn');
