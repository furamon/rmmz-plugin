...
(() => {
  ...
  Game_Troop.prototype.classExpTotal = function (this: Game_Troop): number {
    ...
    const total = this.deadMembers().reduce(
      (r, enemy) => r + (enemy instanceof Game_Enemy ? enemy.classExp() : 0),
      0,
    );
    // 四捨五入
  ...
  AdditionalClass.prototype.changeExp = function (exp: number, show: boolean) {
    ...
    const _newSkills = currentSkills
      .filter((skill: number | MZ.Skill) => {
        ...
        return !_lastSkills.some(
          (lastSkill: number | MZ.Skill) =>
            (typeof lastSkill === "number" ? lastSkill : lastSkill.id) ===
            (typeof skill === "number" ? skill : skill.id),
        );
      })
      .map((skill: number | MZ.Skill) =>
