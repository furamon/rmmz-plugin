...
(function () {
     ...
     * ●アクターを取得
     */
    function getActor(args: { [x: string]: string | true; Actor?: any; VariableActor?: any; }) {
     ...
     * ●全体が対象かどうか？
     */
    function isForParty(args: { [x: string]: string | true; Actor?: any; VariableActor?: any; }) {
    ...
    this,
    this._additionalClassId,
    pUnificationExp,
    pClassLvMaxExp,
    pDefaultMaxLevel,
    pLvUpMessage,
    pZeroLevel,
    pShowMaxLevelMessage,
    pShowBenchMaxLevel,
    pMaxLevelMessage,
    pParamPlusByLevel,
    pParamPlusByTag,
    ...
    Game_Actor.prototype.changeAdditionalClass = function (
        ...
        if (oldClassId) {
            try {
                const oldClass = new AdditionalClass(
    this,
    oldClassId,
    pUnificationExp,
    pClassLvMaxExp,
    ...
    Game_Actor.prototype.currentClass = function (this: Game_Actor): MZ.Class | null {
        if (mForceClassId) {
            // -1で空白を返す
            if (mForceClassId === -1) {
    ...
    Game_Actor.prototype.gainClassExp = function (classExp, ignoreBench) {
        ...
        if (additionalClass) {
            ...
            const newExp =
               Number(additionalClass.currentExp()) + addExpRound;
    ...
    if (pShowLevelOnStatus) {
        ...
        Window_Status.prototype.drawExpInfo = function (x, y) {
            ...
            // 次にレベルアップする経験値
            let nextExp =
                (Number(this.expTotalValue()) +
                Number(this.expNextValue()));
            // 最大レベルの時は-------表記になるので修正
    ...
    Game_Action.prototype.apply = function (target) {
        ...
        // 職業経験値を加算
        const addClassExp = Number(this.item()?.meta.AddClassExp);
        if (addClassExp) {
