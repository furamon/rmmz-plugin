 ...
 * @text ＜ふらもん追加オプション＞
 *
 * @param AutoEquipSwitch
 * @parent <FuramonOption>
 * @text 転職後、最強装備するスイッチ
 * @type switch
 * @desc 転職後、このスイッチがONだと自動で最強装備をします。
 *
 * @param ShowEquipSwitch
 * @parent <FuramonOption>
 * @text 転職後、装備画面を開くスイッチ
 * @type switch
...
class AdditionalClass {
    ...
    get learnings(): MZ.Class.Learning[] {
    ...
    setLevel(): void {
        const exp = this.exp();
        this._level = 1;
        while (!this.isMaxLevel() && (exp as number) >= (this.nextLevelExp() as number)) {
            this._level++;
        }
    }
    ...
    expForLevel(level: number): number {
        if (this._data.meta.NeedsExp) {
            try {
                const needsExp = JSON.parse(this._data.meta.NeedsExp as string);
                if (level <= 1) {
                    return 0;
                }
    ...
    maxLevel(): number {
        ...
        if (metaMaxLevel != null) {
            // Number型として解釈を試みる
            const parsed = Number(metaMaxLevel as any);
            if (!Number.isNaN(parsed)) {
                return parsed;
            }
            // Fallback: 文字列として解釈を試みる
    ...
    changeExp(exp: number, show: boolean): void {
        ...
        while (!this.isMaxLevel() && (this.exp() as number) >= (this.nextLevelExp() as number)) {
            this.levelUp();
        }
        while ((this.exp() as number) < (this.currentLevelExp() as number)) {
            this.levelDown();
        }
        if (this._level > lastLevel) {
            if (show) {
                this.displayLevelUp(skillActor.findNewSkills(lastSkills as MZ.Skill[]));
            }
            if (this.isMaxLevel()) {
    ...
    const pParamPlusByLevel = Boolean(parameters['ParamPlusByLevel'] === 'true');
    const pParamPlusByTag = Boolean(parameters['ParamPlusByTag'] === 'true');
    const pKeepSkill = Boolean(parameters['KeepSkill'] === 'true');
    ...
    const pUseNormalExp = Boolean(parameters['UseNormalExp'] == 'true');
    const pDefaultClassExp = parameters['DefaultClassExp']|| '0';
    const pClassExpMessage = parameters['ClassExpMessage']|| '%1 の%2を獲得！';
    const pClassLvUpLater = Boolean(parameters['ClassLvUpLater'] == 'true');
    const pClassExpSwitch = Number(parameters['ClassExpSwitch'] || 0);
    const pBenchClassExpRate = (parameters['BenchClassExpRate'] || '1.00');
    const pUnificationExp = (parameters['UnificationExp'] != null) ? Boolean(parameters['UnificationExp'] == 'true') : false;
    const pNoDuplicateExp = (parameters['NoDuplicateExp'] != null) ? Boolean(parameters['NoDuplicateExp'] == 'true') : false;
    const pOverwriteClassField = (parameters['OverwriteClassField'] != null) ? Boolean(parameters['OverwriteClassField'] == 'true') : true;
    const pShowLevelOnMenu = parameters['ShowLevelOnMenu'] || '';
    const pShowLevelOnStatus = (parameters['ShowLevelOnStatus'] != null) ? Boolean(parameters['ShowLevelOnStatus'] == 'true') : true;
    ...
    const pShowMaxLevelMessage = (parameters['ShowMaxLevelMessage'] != null) ? Boolean(parameters['ShowMaxLevelMessage'] == 'true') : false;
    const pMaxLevelMessage = parameters['MaxLevelMessage']|| '%1は%2を極めた！';
    const pShowBenchMaxLevel = (parameters['ShowBenchMaxLevel'] != null) ? Boolean(parameters['ShowBenchMaxLevel'] == 'true') : false;

    const pAutoEquipSwitch = Number(parameters['AutoEquipSwitch']|| 0);
    const pShowEquipSwitch = Number(parameters['ShowEquipSwitch']|| 0);
    const pZeroLevel = (parameters['ZeroLevel'] != null) ? Boolean(parameters['ZeroLevel'] == 'true') : false;
     ...
     * ●職業の追加
     */
    PluginManager.registerCommand(PLUGIN_NAME, 'AddClass', function (this: Game_Interpreter, args: { [key: string]: string | true }) {
     ...
     * ●職業の削除
     */
    PluginManager.registerCommand(PLUGIN_NAME, 'RemoveClass', function (this: Game_Interpreter, args: { [key: string]: string | true }) {
        // アクターを取得
        const actor = getActor(args);
        if (!actor) {
            // アクターの指定がない場合は全体を対象化
     ...
     * ●経験値の増減
     */
    PluginManager.registerCommand(PLUGIN_NAME, 'ChangeExp', function (this: Game_Interpreter, args: { [key: string]: string | true }) {
        // イベントコマンドから呼び出されたかどうかの判定
        mCommandFlg = true;
        ...
        if (actor) {
            ...
            if (pUnificationExp && pNoDuplicateExp) {
                ...
                for (const additionalClass of allPartyAdditionalClasses()) {
                    const ac = new AdditionalClass(
                        expActor,
                        additionalClass.id
                    );
                    ac.changeExp(ac.exp() + exp, show);
                }
            } else {
                // 通常時
                for (const member of $gameParty.members()) {
                    const additionalClass = member.additionalClass();
                    if (additionalClass) {
                        additionalClass.changeExp(
                            additionalClass.exp() + exp,
                            show
                        );
                    }
                }
     ...
     * ●レベルの増減
     */
    PluginManager.registerCommand(PLUGIN_NAME, 'ChangeLevel', function (this: Game_Interpreter, args: { [key: string]: string | true }) {
        // イベントコマンドから呼び出されたかどうかの判定
        mCommandFlg = true;
        ...
        if (actor) {
            ...
            if (pUnificationExp && pNoDuplicateExp) {
                ...
                for (const additionalClass of allPartyAdditionalClasses()) {
                    ...
                    ac.changeLevel(ac.level + level, show);
                }
            } else {
                // 通常時
                for (const member of $gameParty.members()) {
    ...
    PluginManager.registerCommand(
        PLUGIN_NAME,
        'GetInformation',
        function (this: Game_Interpreter, args: { [key: string]: string | true }) {
            // アクターを取得
            const actor = getActor(args);
            if (!actor) {
                return;
            ...
            // 追加職業のＩＤ
            const variableAtClass = Number(args.VariableAtClass|| 0);
            if (variableAtClass) {
                if (additionalClass) {
            ...
            // 追加職業のレベル
            const variableAtLv = Number(args.VariableAtLv|| 0);
            if (variableAtLv) {
                if (additionalClass) {
                    $gameVariables.setValue(
                        variableAtLv,
            ...
            // 追加職業の経験値
            const VariableAtExp = Number(args.VariableAtExp|| 0);
            if (VariableAtExp) {
                if (additionalClass) {
                    $gameVariables.setValue(
                        VariableAtExp,
     ...
     * ●アクターを取得
     */
    function getActor(args: { [key: string]: string | true }): Game_Actor | null {
     ...
     * ●全体が対象かどうか？
     */
    function isForParty(args: { [key: string]: string | true }): boolean {
     ...
     * ●経験値共有時の参照用のアクターを取得
     */
    function getExpActor(): Game_Actor {
        // アクター１で固定
        const actor = $gameActors.actor(1);
        if (actor === null) {
            throw new Error("Actor 1 not found.");
        }
        return actor;
    }
     ...
     * ●現在のパーティが就いている全職業を取得
     */
    function allPartyAdditionalClasses(): AdditionalClass[] {
        const additionalClasses: AdditionalClass[] = [];
        const addedClassIds: number[] = [];
        for (const actor of $gameParty.allMembers()) {
            const additionalClass = actor.additionalClass();
            if (
                additionalClass &&
                !addedClassIds.includes(additionalClass.id)
            ) {
                additionalClasses.push(additionalClass);
                addedClassIds.push(additionalClass.id);
            }
        }
     ...
     * ●現在の経験値を取得
     */
    AdditionalClass.prototype.exp = function (this: AdditionalClass): number {
        let exp = this.expActor()._exp[this._id];
     ...
     * ●アクターを取得
     */
    AdditionalClass.prototype.actor = function (this: AdditionalClass): Game_Actor {
        return this._actor;
    };
     ...
     * ●経験値参照用のアクターを取得
     */
    AdditionalClass.prototype.expActor = function (this: AdditionalClass): Game_Actor {
        // ＥＸＰ共有時の場合
        if (pUnificationExp) {
            return getExpActor();
        }
     ...
     * ●経験値からレベルを計算
     */
    AdditionalClass.prototype.setLevel = function (this: AdditionalClass): void {
        const exp = this.exp();
        this._level = 1;
     ...
     * ●現在の経験値を取得
     */
    AdditionalClass.prototype.currentExp = function (this: AdditionalClass, showFlg?: boolean): number | string {
        // 最大レベル時の表示用
        if (showFlg && this.isMaxLevel()) {
            return pClassLvMaxExp;
        }
     ...
     * ●現在レベルまでの経験値を取得
     */
    AdditionalClass.prototype.currentLevelExp = function (this: AdditionalClass): number {
        return this.expForLevel(this._level);
    };
     ...
     * ●次のレベルの経験値を取得
     */
    AdditionalClass.prototype.nextLevelExp = function (this: AdditionalClass, showFlg?: boolean): number | string {
        // 最大レベル時の表示用
        if (showFlg && this.isMaxLevel()) {
            return pClassLvMaxExp;
        }
     ...
     * ●次のレベルまでの経験値を取得
     */
    AdditionalClass.prototype.nextRequiredExp = function (this: AdditionalClass): number {
        return (this.nextLevelExp() as number) - (this.currentExp(false) as number);
    };

    // 一時的に強制変更するための職業ＩＤ
    // declare let mForceClassId: number | null; は @types/furamon.d.ts で宣言済み

    /**
     * ●指定レベルに到達するのに必要な経験値を取得
     */
    AdditionalClass.prototype.expForLevel = function (this: AdditionalClass, level: number): number {
        // <NeedsExp:[n,m,...]> があるかチェック
        if (this._data.meta.NeedsExp) {
            try {
                const needsExp = JSON.parse(this._data.meta.NeedsExp as string);
                if (level <= 1) {
                ...
                // 配列の長さ + 1 が最大レベル
                if (level > needsExp.length + 1) {
                    return this.expForLevel(needsExp.length + 1); // 最大レベルの経験値を返す
                }
                let totalExp = 0;
                for (let i = 0; i < level - 1; i++) {
                    totalExp += needsExp[i] || 0;
                ...
                console.error(
                    'Failed to parse NeedsExp tag for class ' + this.id,
                    e
                );
            }
        }
     ...
     * ●最大レベルかどうか？
     */
    AdditionalClass.prototype.isMaxLevel = function (this: AdditionalClass): boolean {
        return this._level >= this.maxLevel();
    };
     ...
     * ●最大レベルを取得
     */
    AdditionalClass.prototype.maxLevel = function (this: AdditionalClass): number {
        // <NeedsExp>
        if (this._data.meta.NeedsExp) {
            try {
                const needsExp = JSON.parse(this._data.meta.NeedsExp as string);
        ...
        if (metaMaxLevel != null) {
            ...
            // Fallback: parse integer from string
            const parsedInt = parseInt(String(metaMaxLevel), 10);
            if (!Number.isNaN(parsedInt)) {
                return parsedInt;
            }
            // If it's unparseable (e.g. arbitrary string), fall through to defaults
        } else if (pDefaultMaxLevel) {
            return pDefaultMaxLevel;
        }
     ...
     * ※expは加算済みの経験値
     */
    AdditionalClass.prototype.changeExp = function (this: AdditionalClass, exp: number, show: boolean): void {
        const expActor = this.expActor();
        const skillActor = this.actor();
        const classId = this.id;
     ...
     * ●レベルを変更
     */
    AdditionalClass.prototype.changeLevel = function (this: AdditionalClass, level: number, show: boolean): void {
        level = level.clamp(1, this.maxLevel());
        this.changeExp(this.expForLevel(level), show);
    };
     ...
     * ●レベルアップ
     */
    AdditionalClass.prototype.levelUp = function (this: AdditionalClass): void {
        const actor = this.actor();
     ...
     * ●レベルダウン
     */
    AdditionalClass.prototype.levelDown = function (this: AdditionalClass): void {
        this._level--;
    };
     ...
     * ●レベルアップメッセージの表示
     */
    AdditionalClass.prototype.displayLevelUp = function (this: AdditionalClass, newSkills: MZ.Skill[]): void {
        if (pLvUpMessage) {
            const actor = this.actor();
            const displayLevel = pZeroLevel ? this._level - 1 : this._level;
            const text = pLvUpMessage.format(
     ...
     * ●レベルアップ限界の表示
     */
    AdditionalClass.prototype.displayLevelMax = function (this: AdditionalClass, show: boolean): void {
        // 表示しない場合
        if (!pShowMaxLevelMessage) {
            return;
        }
        ...
        const displayLevel = pZeroLevel ? this._level - 1 : this._level;
        const text = pMaxLevelMessage.format(
            actor.name(),
            this.name,
            displayLevel
        );
    ...
    // 共通処理
    //-----------------------------------------------------------------------------
    Game_Actor.prototype.additionalClass = function (this: Game_Actor): AdditionalClass | undefined {
     ...
     * 【独自】追加職業を取得（JSON）
     */
    Game_Actor.prototype.additionalClassObject = function (this: Game_Actor): MZ.Class | undefined {
        return this._additionalClassId ? $dataClasses[this._additionalClassId] : undefined;
    };

    /**
     * 【独自】追加職業を追加
     */
    Game_Actor.prototype.changeAdditionalClass = function (this: Game_Actor, classId: number): void {
        const oldClassId = this._additionalClassId;
        ...
        // 転職後処理
        if (classId && $gameSwitches.value(pAutoEquipSwitch)) {
            this.optimizeEquipments();
        }
        if (classId && $gameSwitches.value(pShowEquipSwitch)) {
     ...
     * 【独自】追加職業の解除
     */
    Game_Actor.prototype.leaveAdditionalClass = function (this: Game_Actor): void {
        this.changeAdditionalClass(0);
    };

    /**
     * ●転職時も保持するスキルかどうか？
     */
    function isKeepSkill(skillId: number): boolean {
        const skillData = $dataSkills[skillId];

        // スキル毎の指定がある場合は優先
        const metaKeepSkill = skillData.meta.KeepSkill != null ? Boolean(skillData.meta.KeepSkill == 'true') : pKeepSkill;
    ...
    Game_Actor.prototype.currentClass = function (this: Game_Actor): MZ.Class {
        if (mForceClassId) {
            ...
            if (mForceClassId === -1) {
                ...
                return { id: -1, name: "", note: "", learnings: [], params: [[0]], meta: {}, expParams: [], traits: [] } as MZ.Class;
            }
     ...
     * 【独自】全追加職業のスキルを習得
     */
    Game_Actor.prototype.setAllAdditionalClassesSkills = function (this: Game_Actor): void {
        // 追加職業のスキルを習得
     ...
     * 【独自】追加職業のスキルを習得
     */
    Game_Actor.prototype.setAdditionalClassSkills = function (this: Game_Actor, additionalClass: AdditionalClass): void {
        // 職業のレベルを取得
    ...
    Game_Actor.prototype.initSkills = function (this: Game_Actor): void {
    ...
    Game_Actor.prototype.traitObjects = function (this: Game_Actor): DataManager.TraitObject[] {
     ...
     * ●アクターが追加職業についているか？
     */
    Game_Actor.prototype.isAdditionalClass = function (this: Game_Actor, gameClass: MZ.Class): boolean {
        if (!gameClass) {
            return false;
        }
        return this.isAdditionalClassId(gameClass.id);
    };

    /**
     * ●アクターが追加職業についているか？（ＩＤ）
     */
    Game_Actor.prototype.isAdditionalClassId = function (this: Game_Actor, classId: number): boolean {
        return this._additionalClassId === classId;
    };

    if (pParamPlusByLevel || pParamPlusByTag) {
        const _Game_Actor_paramPlus = Game_Actor.prototype.paramPlus;
        Game_Actor.prototype.paramPlus = function (this: Game_Actor, paramId: number): number {
            let value = _Game_Actor_paramPlus.call(this, paramId);
            const additionalClass = this.additionalClass();

            // 追加職業のレベルによる能力値上昇
            if (pParamPlusByLevel && additionalClass) {
            ...
            if (pParamPlusByTag) {
                ...
                const pattern = new RegExp(`<Add${paramName}:(-?\d+)>`, 'i');

                let maxValue = 0;

                const objects = this.traitObjects();
                // traitObjects() は TraitObject 型の配列なのでそのまま扱う。
                // スキルは別配列に集め、後で結合してノートを走査する（型の混在を避ける）。
                const noteObjects = objects.slice(); // TraitObject[] のコピー
                const skillNoteObjects: any[] = [];
                for (const skillEntry of this.skills() as (number | MZ.Skill)[]) {
                    if (typeof skillEntry === 'number') {
                        const skill = $dataSkills[skillEntry];
                        if (skill) {
                ...
                for (const obj of noteObjects.concat(skillNoteObjects)) {
                    if (obj && obj.note) {
                        const match = obj.note.match(pattern);
                        if (match) {
                            const addValue = parseInt(match[1], 10);
            ...
            if (paramName === 'Mhp' || paramName === 'Mmp') {
                    value = Math.round(value * (1 + maxValue / 100)); // Multiply for MHP/MMP
                } else {
                    value += maxValue; // Add for others
                }
            }
    ...
    BattleManager.displayExp = function (this: typeof BattleManager): void {
    ...
    BattleManager.gainClassExp = function (this: typeof BattleManager): void {
        const classExp = this._rewards.classExp;
        ...
        if (pUnificationExp && pNoDuplicateExp) {
            ...
            for (const additionalClass of allPartyAdditionalClasses()) {
                ...
                const newExp =
                    additionalClass.currentExp() as number +
     ...
     * 【独自】職業経験値の獲得（アクター）
     */
    Game_Actor.prototype.gainClassExp = function (this: Game_Actor, classExp: number, ignoreBench?: boolean): void {
        const additionalClass = this.additionalClass();
        if (additionalClass) {
            let addExp = classExp;
            // 控えメンバー倍率
            ...
            const newExp = (additionalClass.currentExp() as number) + addExpRound;
     ...
     * 【独自】職業経験値の獲得率
     */
    Game_Actor.prototype.finalClassExpRate = function (this: Game_Actor): number {
        let rate = 1;
        ...
        for (const object of this.traitObjects()) {
            ...
            if (classExpRate != null) {
                rate *= eval(classExpRate as string) / 100;
            }
        }

        return (
            rate * (this.isBattleMember() ? 1 : this.benchMembersClassExpRate())
        );
    };
     ...
     * 【独自】控えメンバーの職業経験値比率
     */
    Game_Actor.prototype.benchMembersClassExpRate = function (this: Game_Actor): number {
        if (pBenchClassExpRate !== undefined) {
            const a = this; // eval計算用
            return eval(pBenchClassExpRate as string);
        }
     ...
     * 　こちらの末尾に追加
     */
    // const _BattleManager_displayExp = BattleManager.displayExp; // 上で定義済み
     ...
     * ●職業経験値が有効かどうか？
     */
    Game_System.prototype.isClassExpEnabled = function (this: Game_System): boolean {
        // 有効スイッチがオフの場合は変更禁止
    ...
    if (pUnificationExp) {
         ...
         * ●共有用アクターの経験値を反映する。
         */
        Game_Actor.prototype.setUnificationExp = function (this: Game_Actor): void {
            const expActor = getExpActor();
    ...
    if (pOverwriteClassField) {
         ...
         * 【上書】職業の表示
         */
        Window_StatusBase.prototype.drawActorClass = function (
            this: Window_StatusBase,
            actor: Game_Actor,
            x: number,
            y: number,
            width?: number
        ...
        if (pShowLevelOnMenu) {
            const _Window_MenuStatus_drawActorClass = Window_MenuStatus.prototype.drawActorClass;
            /**
             * 【上書】職業の表示
             */
            Window_MenuStatus.prototype.drawActorClass = function (
                this: Window_MenuStatus,
                actor: Game_Actor,
                x: number,
                y: number,
                width?: number
            ) {
                _Window_MenuStatus_drawActorClass.call(this, actor, x, y, width);
                ...
                const additionalClass = actor.additionalClass();
                            if (additionalClass && !additionalClass._data.meta.NoGrow) {
                                ...
                                if (pShowLevelOnMenu === 'simple') {
                                    this.drawText(String(displayLevel), x, y, 30, 'right');
    ...
    if (pShowLevelOnStatus) {
         ...
         * ●ステータス画面のブロック１
         */
        Window_Status.prototype.drawBlock1 = function (this: Window_Status): void {
         ...
         * ●アクターレベルの描画
         */
        Window_Status.prototype.drawActorLevel = function (this: Window_Status, actor: Game_Actor, x: number, y: number): void {
            this.changeTextColor(ColorManager.systemColor());
         ...
         * ●経験値の描画
         */
        Window_Status.prototype.drawExpInfo = function (this: Window_Status, x: number, y: number): void {
            this.changeTextColor(ColorManager.systemColor());
            this.drawText(
                TextManager.exp,
                x - pNormalExpWidth * 2 - 30,
                y,
            ...
            // 現在の経験値
            this.drawText(
                String(this.expTotalValue()),
                x - pNormalExpWidth - 15,
                y,
                this.innerWidth - this.itemPadding(),
                'right'
            );
            ...
            // 次にレベルアップする経験値
            let nextExp: string | number = (this.expTotalValue() as number) + (this.expNextValue() as number);
            // 最大レベルの時は-------表記になるので修正
         ...
         * ●ブロック２（２行目）の描画
         */
        Window_Status.prototype.drawBlock2 = function (this: Window_Status): void {
        ...
        Window_Status.prototype.drawClassInfo = function (
            ...
            // 現在の経験値
            this.drawText(
                String(additionalClass.currentExp(true)),
                x - pClassExpWidth - 15,
                y,
                this.innerWidth - this.itemPadding(),
                'right'
            ...
            // 次にレベルアップする経験値
            this.drawText(
                String(additionalClass.nextLevelExp(true)),
                x,
                y,
                this.innerWidth - this.itemPadding(),
                'right'
         ...
         * ●基本情報の出力
         */
        Window_Status.prototype.drawBasicInfo = function (this: Window_Status, x: number, y: number): void {
    ...
    Window_StatusBase.prototype.drawAdditionalClassLevel = function (
        ...
        if (additionalClass) {
            ...
            const displayLevel = pZeroLevel
                ? additionalClass.level - 1
                : additionalClass.level;
            this.drawText(String(displayLevel), x + 44, y, 36, 'right');
        }
    };
    ...
    Game_Action.prototype.apply = function (this: Game_Action, target: Game_Battler): void {
        ...
        if (addClassExp) {
            ...
            if (result.isHit() && target instanceof Game_Actor) {
                target.gainClassExp(eval(addClassExp as string) as number, true);
                // Game_Actor を直接 Game_Battler にキャストすると型の重複チェックで警告が出るため
                // 一旦 unknown を経由してキャストする。
                this.makeSuccess(target as unknown as Game_Battler);
            }
        }
     ...
     * ●効果適用判定
     */
    const _Game_Action_hasItemAnyValidEffects =
        Game_Action.prototype.hasItemAnyValidEffects;
    Game_Action.prototype.hasItemAnyValidEffects = function (this: Game_Action, target: Game_Battler): boolean {
     ...
     */
    const _Game_Actor_traitBattlerObjects =
        Game_Actor.prototype.traitBattlerObjects;
    Game_Actor.prototype.traitBattlerObjects = function (this: Game_Actor): DataManager.TraitObject[] {
    ...
    let _lastSkills: number[] = [];

    const _AdditionalClass_changeExp = AdditionalClass.prototype.changeExp;
    AdditionalClass.prototype.changeExp = function (this: AdditionalClass, exp: number, show: boolean) {
        _lastSkills = this.actor().skills({ includeHasAbilitySkills: true }) as number[];
    ...
    AdditionalClass.prototype.displayLevelUp = function (this: AdditionalClass, newSkills: MZ.Skill[]) {
        // 現在のスキル一覧は number の配列か Skill オブジェクトの配列のいずれかになる可能性があるため、
        // まず ID に正規化してから差分を取り、$dataSkills にアクセスするようにする。
        const currentSkills = this.actor().skills({ includeHasAbilitySkills: true }) as (number | MZ.Skill)[];
        const _newSkills = currentSkills
            .map((s) => (typeof s === 'number' ? s : (s as MZ.Skill).id))
            .filter((id: number) => !_lastSkills.includes(id))
            .map((id: number) => $dataSkills[id]);
