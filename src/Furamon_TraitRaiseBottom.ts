 ...
 * @author Furamon
 *
 * @help 能力値乗算特徴がアクターや職業、装備などに複数ついている場合、
 * 一番高いものを返すよう挙動を改変します。
 * 例えば
 * - 魔法力が120%の職業のアクターが魔法力150%の装備をつけていると150%のみ反映
 * - 魔法防御が80%の職業のアクターが魔法防御110%の装備をつけていると
 * 110%のみ反映
 * という挙動、要はFF5の装備系アビリティっぽくなります。
 * ただし最大HPだけは加算処理(130%と120%なら150%になる)か
 * 従来の乗算処理になります（パラメータで選べます）。
 * また、装備能力上昇値がアクター側の補正を受けないように
 * できる機能もあります。(ステートは普通に乗算)
 *
 * -----------------------------------------------------------------------------
 * # あてんしょん #
 * -----------------------------------------------------------------------------
 * Game_Actor.prototype.paramやparamRateあたりを盛大に書き換えます。
 * 可能な限り上の方で運用したほうが競合しないと思います。
 *
 * -----------------------------------------------------------------------------
 * # 謝辞 #
 * -----------------------------------------------------------------------------
...
(function () {
     ...
     // ステートの特徴計算
     Game_Actor.prototype.getStateParamRate = function (
         paramId: number
     ): number {
         // 現在かかっているステートを取得
     ...
     Game_Actor.prototype.param = function (paramId: number) {
         ...
         let value =
             (baseValue * actorClassPassiveRate + plusValue) *
             stateRate *
             buffRate;
             ...
             if (obj && obj.traits) {
                 ...
                 if ((!isEquip || isPassive) && !isCurrentState) {
                     ...
                     if (Array.isArray(obj.traits)) {
                         const paramTraits = obj.traits.filter(
                             (trait) =>
                                 trait.code === Game_BattlerBase.TRAIT_PARAM &&
                                 trait.dataId === paramId
                         );
