...
(() => {
    const hasDotMove = PluginManager._scripts.some(
        (name) => name.toLowerCase() === 'dotmovesystem'
    );
    const hasEventRange = PluginManager._scripts.some(
        (name) => name.toLowerCase() === 'nuun_eventrange'
    );
    ...
    const startRangeEvents = (
        ...
        const mapAny = $gameMap as MapWithRange;
        const candidates =
            mapAny.eventsRangeEventPlayerXy?.(tileX, tileY) ?? [];
        if (candidates.length === 0) {
            return;
        }
        for (const candidate of candidates) {
            const event = candidate as Game_Event & EventRangeEvent;
    ...
    Game_Player.prototype.startMapEventFront = function (
        this: Game_Player & PlayerWithRange,
        x: number,
        y: number,
        d: number,
        ...
        this.setDistanceFrom?.(0, 0);
        originalStartMapEventFront.call(
            this,
            x,
            y,
            d,
    ...
    Game_CharacterBase.prototype.moveCallback = function (
        ...
        const event = this as Game_Event &
            EventRangeEvent & { _trigger?: number; x: number; y: number };
        if (typeof event.range !== 'function') {
            return;
        }
        const hasRangeTag =
            typeof event.getEventRangeTag === 'function' &&
        ...
        const player = $gamePlayer as PlayerWithRange;
