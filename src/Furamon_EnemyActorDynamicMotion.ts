...
(function () {
    ...
    const existNRP_DynamicMotionMZ = PluginManager._scripts.some(function (
        scriptName
    ) {
    ...
    if (!existNRP_DynamicMotionMZ) {
        console.warn(
            '[Furamon] NRP_DynamicMotionMZ is required for EnemyActorDynamicMotion'
        );
        return;
    }

    // NRP_EnemyCollapseのプラグインパラメータ取得
    const prmEnemyCollapse =
        PluginManager._scripts.includes('NRP_EnemyCollapse') &&
        PluginManager.parameters('NRP_EnemyCollapse');

    // SVアクター敵かどうかを判定する関数
    function isSvActorEnemy(battler: Game_Enemy) {
     ...
     */
    Game_Enemy.prototype.performAttackDynamicMotion = function (
        weaponId,
        weaponType
    ) {
     ...
     * ●SVアクター敵の独自消滅演出処理
     */
    const _Sprite_Enemy_startOriginalCollapse =
        Sprite_Enemy.prototype.startOriginalCollapse;
     ...
     * ●SVアクター敵の独自消滅演出更新
     */
    const _Sprite_Enemy_updateOriginalCollapse =
        Sprite_Enemy.prototype.updateOriginalCollapse;
            ...
            if (collapseData.CollapseType == 'Sink') {
                ...
                if (
                    ...
                    if (visibleHeight > 0) {
                        mainSprite.setFrame(
                            currentFrame.x,
                            currentFrame.y, // Y座標はそのまま（上から表示）
                            currentFrame.width,
                            visibleHeight // 高さを減らして下をカット
                        );
                    } else {
                        // 完全に沈んだ場合は非表示
                        mainSprite.setFrame(
    ...
    function getBattlerSprite(battler: Game_Enemy) {
