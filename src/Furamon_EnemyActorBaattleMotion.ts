...
(function () {
    ...
    if (typeof Sprite_SvActor === 'undefined') {
        console.warn(`${pluginName}: Furamon_EnemyActorAnimation not found`);
        return;
    }
    ...
    Sprite_SvActor.prototype.updateFrameBMMZ = function (bitmap) {
        // BattleMotionMZ処理
    ...
    // 終端フレームの青色要素を取得
    Sprite_SvActor.prototype.getEndFrameBlueValue = function (bitmap, x, y) {
        try {
            const canvas = bitmap.canvas || bitmap._canvas;
            if (!canvas) return 0;
