...
(() => {
    ...
    function findSettingForWindow(windowObject: WindowLike): TweenSetting | null {
    ...
    const TweenManager = {
        ...
        parseMoveValue(value: string, baseValue: number): number {
            if (value.includes('%')) {
    ...
    Scene_Base.prototype.addWindow = function (windowObject) {
        ...
        const setting = findSettingForWindow(windowObject as WindowLike);
        if (setting) {
            this._tweenableWindows.push({ window: windowObject as WindowLike, setting: setting });
            TweenManager.applyOpen(windowObject as WindowLike, setting);
        }
    };
