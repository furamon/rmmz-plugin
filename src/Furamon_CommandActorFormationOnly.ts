...
(() => {
    ...
    Window_FormationMember.prototype.isFormationChangeActorEnabled = function (
        actor
    ) {
        return this.isChangeActorActive(actor);
    };

    const _Window_FormationBattleMember_initialize =
        Window_FormationBattleMember.prototype.initialize;
    Window_FormationBattleMember.prototype.initialize = function (
        rect,
        formation
    ) {
        ...
        const _Window_FormationMember_initialize =
        Window_FormationMember.prototype.initialize;
    Window_FormationMember.prototype.initialize = function (
        rect,
        formation
    ) {
    ...
    const _Window_StatusBase_getFormationSelectActor =
        Window_StatusBase.prototype.getFormationSelectActor;
    ...
    // ターンが回っている味方は背景を緑に
    const _Window_StatusBase_drawBackGroundActor =
        Window_StatusBase.prototype.drawBackGroundActor;
    Window_StatusBase.prototype.drawBackGroundActor = function (index) {
        ...
        if ($gameParty.inBattle()) {
            if (actor === BattleManager._subject as unknown as Game_Actor) {
                this.contentsBack.fillRect(
                    rect.x,
                    y - 4,
                    rect.width,
                    51,
                    ColorManager.textColor(11)
                );
            }
        }
